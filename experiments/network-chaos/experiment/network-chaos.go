...
import (
	...
	"github.com/litmuschaos/m-agent/pkg/network"
...
// NetworkChaos listens for the client actions and executes them as appropriate
func NetworkChaos(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case "CHECK_STEADY_STATE":
			if err := network.NetworkChaosPreRequisiteCheck(payload); err != nil {
		...
		case "EXECUTE_EXPERIMENT":
			err = network.InjectNetworkChaos(payload)
		...
		// case "CHECK_LIVENESS":
		// 	if err := network.CheckProcessLiveness(cmd.Process.Pid); err != nil {
		...
		case "REVERT_EXPERIMENT":
			if err := network.RevertNetworkChaos(payload); err != nil {
